#!/usr/bin/env python3
"""Stop hook: checks context window usage and reports to the user.

Runs automatically after every Claude response via the Stop hook.
Reads settings from .claude/skills/check-context/settings.json.
Returns structured output that Claude relays to the user.
"""
...
def main():
    ...
    # Critical zone: block and force handover
    if percent > 85:
        print(json.dumps({
            "decision": "block",
            "reason": (
                f"CONTEXT CRITICAL: {percent}% used ({used_k}k/{total_k}k tokens). "
                f"You MUST stop now. Tell the user:\n"
    ...
    # At or above threshold: warn and advise
    print(json.dumps({
        "decision": "block",
        "reason": (
